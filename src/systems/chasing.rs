...
pub fn chasing(#[resource] map: &Map, ecs: &SubWorld, commands: &mut CommandBuffer) {
    ...
    let dijakstra_map = DijkstraMap::new(SCREEN_WIDTH, SCREEN_HEIGHT, &search_targets, map, 1024.0);
    ...
    movers.iter(ecs).for_each(|(entity, pos, _, fov)| {
        ...
        if let Some(destination) = DijkstraMap::find_lowest_exit(&dijakstra_map, idx, map) {
